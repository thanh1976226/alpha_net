    ...
    .run([
      ...
      function ($templateCache) {
        $templateCache.put('template/treeGrid/treeGrid.html',
          ...
          "   <thead>\n" +
          "     <tr>\n" +
          "       <th><a ng-if=\"expandingProperty.sortable\" ng-click=\"sortBy(expandingProperty)\"></a><span ng-if=\"!expandingProperty.sortable\"></span><i ng-if=\"expandingProperty.sorted\" class=\"{{expandingProperty.sortingIcon}} pull-right\"></i></th>\n" +
	  "       <th class=\"{{col.colclass}}\" ng-hide=\"col.field=='_ModerationStatus' && !$root.ListInfo.Moderation\"  ng-repeat=\"col in colDefinitions\" style='width:{{col.width}}'  ><a ng-if=\"col.sortable\" ng-click=\"sortBy(col)\">{{col.displayName || col.field}}</a><span ng-if=\"!col.sortable\">{{col.displayName || col.field}}</span><i ng-if=\"col.sorted\" class=\"{{col.sortingIcon}} pull-right\"></i></th>\n" +
          "     </tr>\n" +
          "   </thead>\n" +
          "   <tbody>\n" +
          "     <tr ng-repeat=\"row in tree_rows | searchFor:$parent.filterString:expandingProperty:colDefinitions track by row.branch.uid\"\n" +
          "       ng-class=\"'level-' + {{ row.level }} + (row.branch.selected ? ' active':'')\"  ng-click=\"user_clicks_branch(row.branch)\" class=\"tree-grid-row\">\n" +
          ...
          "       </td>\n" +
          "       <td ng-repeat=\"col in colDefinitions\" ng-hide=\"col.field=='_ModerationStatus' && !$root.ListInfo.Moderation\" >\n" +
    ...
    .directive('treeGrid', ['$rootScope',
      ...
      function ($rootScope,$timeout,
        ...
        return {
          ...
          link       : function (scope, element, attrs) {
            var error, expandingProperty,list_info, expand_all_parents, expand_level, for_all_ancestors, for_each_branch, get_parent, n, on_treeData_change, select_branch, selected_branch, tree;
			$timeout(function(){ 
				//console.log(scope);
			},0);
