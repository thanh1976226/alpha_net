...
directiveModule.directive("ngDropdownMultiselect", ["$filter", "$document", "$compile", function ($filter, $document)
{
    return {
        restrict: "AE",
        scope: { selectedModel: "=", options: "=", extraSettings: "=" },
        ...
        link: function ($scope) {
            function getFindObj(id) {
                var findObj = {};
                return "" === $scope.settings.externalIdProp ? findObj[$scope.settings.idProp] = id : findObj[$scope.settings.externalIdProp] = id,
                    findObj
            }
            $scope.toggleDropdown = function () {
                ...
                $scope.checkboxClick = function ($event, id) {
                    $scope.setSelectedItem(id), $event.stopImmediatePropagation()
                },
                $scope.settings = { dynamicTitle: !0, defaultText: "Select", closeOnBlur: !0, displayProp: "label", idProp: "id", externalIdProp: "id" },
                angular.extend($scope.settings, $scope.extraSettings || []),
                $scope.settings.closeOnBlur && $document.on("click", function (e) {
                    for (var target = e.target.parentElement, parentFound = !1; angular.isDefined(target) && null !== target && !parentFound;)_.contains(target.classList, "multiselect-parent") && !parentFound && (parentFound = !0), target = target.parentElement; parentFound || $scope.$apply(function () { $scope.open = !1 })
                }),
                $scope.getButtonText = function () {
                if ($scope.settings.dynamicTitle) {
                    var totalSelected = angular.isDefined($scope.selectedModel) ? $scope.selectedModel.length : 0; return 0 === totalSelected ? $scope.settings.defaultText : /*totalSelected + " selected"*/  $scope.selectedModel
                } return $scope.settings.defaultText
                }, $scope.getPropertyForObject = function (object, property) { return object.hasOwnProperty(property) ? object[property] : "" }, $scope.selectAll = function () { $scope.deselectAll(), angular.forEach($scope.options, function (value) { $scope.setSelectedItem(value[$scope.settings.idProp], !0) }) }, $scope.deselectAll = function () { $scope.selectedModel = [] }, $scope.setSelectedItem = function (id, dontRemove) { dontRemove = dontRemove || !1; var findObj = getFindObj(id), exists = -1 !== _.findIndex($scope.selectedModel, findObj); if (!dontRemove && exists) $scope.selectedModel.splice(_.findIndex($scope.selectedModel, findObj), 1); else if (!exists) if ("" === $scope.settings.externalIdProp) { var fullObjFind = getFindObj(id), fullObj = _.find($scope.options, fullObjFind); $scope.selectedModel.push(fullObj) } else $scope.selectedModel.push(findObj); return !1 }, $scope.isChecked = function (id) { return -1 !== _.findIndex($scope.selectedModel, getFindObj(id)) ? !0 : !1 }
        }
    }
}]);