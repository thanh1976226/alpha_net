(function() {
  ...
  var tmpl = [
    ...
    '    <label class="timeLabel">Thời gian:</label> <span class="timeValue">{{("0"+inputHour).slice(-2)}} : {{("0"+inputMinute).slice(-2)}}</span><br/>',
    '    <label class="hourLabel">Giờ:</label> <input class="hourInput" type="range" min="0" max="23" ng-model="inputHour" ng-change="updateNgModel()" />',
    '    <label class="minutesLabel">Phút:</label> <input class="minutesInput" type="range" min="0" max="59" ng-model="inputMinute"  ng-change="updateNgModel()"/> ',
    '  </div> ',
    '</div>'].join("\n");
    ...
    var linkFunc = function(scope, element, attrs, ctrl) { //jshint ignore:line
      ...
      scope.updateNgModel = function(day) {
        ...
        if (attrs.ngModel) { 
		  ...
		  //console.log(moment(scope.selectedDate).format(dateFormat));
          if (elScope.$eval(attrs.ngModel) && elScope.$eval(attrs.ngModel).constructor.name === 'Date') {
			
            dateValue =moment(scope.selectedDate).format(dateFormat);// new Date(dateFilter(scope.selectedDate, dateFormat));
          } else {
            dateValue =moment(scope.selectedDate).format(dateFormat);// dateFilter(scope.selectedDate, dateFormat);
          }
		    
		var temp =	 "\/Date(" + moment(dateValue, dateFormat).valueOf() + ")\/";
		//console.log(temp);
          elScope.$eval(attrs.ngModel + '= date', {date: temp});
        }
      };
    ...
    return {
      ...
      link: function(scope, element, attrs, ctrl) {
		    ...
		    ctrl.$formatters.unshift(function (modelValue) { 
			  ...
			  if(modelValue && modelValue.toString().indexOf('Date')>-1){
				  var dmodelValue = moment(modelValue); 
				  modelValue=dmodelValue.format(format)
			  } 	
              return modelValue;
            });
            ...
            ctrl.$parsers.unshift(function (viewValue) { 
				  ...
				  if(viewValue && viewValue.toString().indexOf('Date')>-1){
					  var dmodelValue = moment(viewValue); 
					  viewValue=dmodelValue.format(format)
				  } 	
				 if(!moment(viewValue, format).isValid()){
					 return null;
				 } 
				var viewValue =	 "\/Date(" + moment(viewValue, format).valueOf() + ")\/"; 
				return viewValue;
            }); 
