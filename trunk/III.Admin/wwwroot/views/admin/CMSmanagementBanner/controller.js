var ctxfolder = "/views/admin/CMSmanagementBanner";
...
app.factory('dataservice', function ($http) {
    ...
    return {
        getUser: function (callback) {
            $http.post('/Admin/CMSManagementBanner/GetUser').success(callback);
        },
        getActivityType: function (callback) {
            $http.post('/Admin/CMSManagementBanner/GetActivityType').success(callback);
        },
        getAsset: function (callback) {
            $http.post('/Admin/CMSManagementBanner/GetAsset/').success(callback);
        },
        getItem: function (data, callback) {
            $http.post('/Admin/CMSManagementBanner/GetItem/', data).success(callback);
        },
        insert: function (data, callback) {
            $http.post('/Admin/CMSManagementBanner/Insert/', data).success(callback);
        },
        update: function (data, callback) {
            $http.post('/Admin/CMSManagementBanner/Update/', data).success(callback);
        },
        delete: function (data, callback) {
            $http.post('/Admin/CMSManagementBanner/Delete/' + data).success(callback);
        },
...
app.controller('index', function ($scope, $rootScope, $compile, $uibModal, DTOptionsBuilder, DTColumnBuilder, DTInstances, dataservice, $translate) {
    ...
    vm.dtOptions = DTOptionsBuilder.newOptions()
        .withOption('ajax', {
            url: "/Admin/CMSManagementBanner/Jtable",
    ...
    vm.dtColumns.push(DTColumnBuilder.newColumn('ActCode').withTitle('{{ "ID" | translate }}').renderWith(function (data, type) {
        return "1";
    }));
    vm.dtColumns.push(DTColumnBuilder.newColumn('ActTitle').withTitle('{{ "Tiêu đề" | translate }}').renderWith(function (data, type) {
        return "cảnh sát biển";
    }));
    vm.dtColumns.push(DTColumnBuilder.newColumn('ActType').withTitle('{{ "Ảnh" | translate }}').renderWith(function (data, type) {
        for (var i = 0; i < $rootScope.ActivityType.length; i++) {
            if (data === $rootScope.ActivityType[i].Code) {
                return $rootScope.ActivityType[i].Name;
            }
        }
        return "<img style='height:100px;width:100px;' src='../images/default/Background.png' / >";
    }));
    vm.dtColumns.push(DTColumnBuilder.newColumn('ActNote').withTitle('{{ "Hiển thị" | translate }}').renderWith(function (data, type) {
        return data;
    }));
    vm.dtColumns.push(DTColumnBuilder.newColumn('ActMember').withTitle('{{ "Xếp" | translate }}').renderWith(function (data, type) {
        if (data != null) {
            var listUser = "";
            var list = data.split(',');
    ...
    vm.dtColumns.push(DTColumnBuilder.newColumn('ActNote').withTitle('{{ "Ngày tạo" | translate }}').renderWith(function (data, type) {
        return "1/1/2000";
    }));
    vm.dtColumns.push(DTColumnBuilder.newColumn('ActNote').withTitle('{{ "Ngày sửa" | translate }}').renderWith(function (data, type) {
        return "1/1/2001";
    }));
