...
app.factory('dataservice', function ($http) {
    ...
    return {
        getAllType: function (callback) {
            $http.post('/Admin/MaterialsType/GetAllData').success(callback);
        },
        insert: function (data, callback) {
            $http.post('/Admin/MaterialsType/Insert/', data).success(callback);
        },
        update: function (data, callback) {
            $http.post('/Admin/MaterialsType/Update/', data).success(callback);
        },
        getData: function (data, callback) {
            $http.post('/Admin/MaterialsType/GetData/' + data).success(callback);
        },
        delete: function (data, callback) {
            $http.post('/Admin/MaterialsType/Delete/' + data).success(callback);
        }
    }
});
app.controller('Ctrl_ESEIM', function ($scope, $rootScope, $compile, $uibModal, DTOptionsBuilder, DTColumnBuilder, DTInstances, dataservice, $cookies, $translate) {
    ...
    $rootScope.$on('$translateChangeSuccess', function () {
        ...
        $rootScope.validationOptions = {
            ...
            messages: {
                Code: {
                    required: caption.COM_ERR_REQUIRED.replace("{0}", caption.MATERIAL_TYPE_CURD_LBL_CODE),
                    maxlength: caption.COM_ERR_EXCEED_CHARACTERS.replace("{0}", caption.MATERIAL_TYPE_CURD_LBL_CODE).replace("{1}", "100")
                },
                Name: {
                    required: caption.COM_ERR_REQUIRED.replace("{0}", caption.MATERIAL_TYPE_CURD_LBL_NAME),
                    maxlength: caption.COM_ERR_EXCEED_CHARACTERS.replace("{0}", caption.MATERIAL_TYPE_CURD_LBL_NAME).replace("{1}", "255")
                }
            }
        }
    });
...
app.controller('index', function ($scope, $rootScope, $compile, $uibModal, DTOptionsBuilder, DTColumnBuilder, DTInstances, dataservice, $translate) {
    ...
    vm.dtColumns.push(DTColumnBuilder.newColumn('code').withTitle('{{"MATERIAL_TYPE_LIST_COL_CODE" | translate}}').renderWith(function (data, type) {
        return data;
    }));
    vm.dtColumns.push(DTColumnBuilder.newColumn('name').withTitle('{{"MATERIAL_TYPE_LIST_COL_NAME" | translate}}').renderWith(function (data, type) {
        return data;
    }));
    vm.dtColumns.push(DTColumnBuilder.newColumn('description').withTitle('{{"MATERIAL_TYPE_LIST_COL_DESCRIPTION" | translate}}').renderWith(function (data, type) {
        return data;
    }));
    vm.dtColumns.push(DTColumnBuilder.newColumn('action').notSortable().withOption('sClass', 'nowrap').withOption('sWidth', '40px').withTitle('{{"MATERIAL_TYPE_LIST_COL_ACTION" | translate}}').renderWith(function (data, type, full) {
...
app.controller('add', function ($scope, $rootScope, $compile, $uibModal, $uibModalInstance, dataservice) {
    ...
    $scope.submit = function () {
        if ($scope.addform.validate()) {
            ...
            if (msg.Status) {
                App.toastrError(caption.COM_VALIDATE_ITEM.replace('{0}', caption.MATERIAL_TYPE_CURD_LBL_CODE));
                return;
            }
