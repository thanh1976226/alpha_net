...
app.factory('dataservice', function ($http) {
    ...
    return {
        ...
        getCurrency: function (callback) {
            $http.post('/Admin/FundCurrency/GetCurrency/').success(callback);
        },
        ...
        setDefaultPayment: function (data, callback) {
            $http.post('/Admin/FundCurrency/SetDefaultPayment/' + data).success(callback);
        },
        unSetDefaultPayment: function (data, callback) {
            $http.post('/Admin/FundCurrency/UnSetDefaultPayment/' + data).success(callback);
        },
    }
});
...
app.controller('index', function ($scope, $rootScope, $compile, $uibModal, DTOptionsBuilder, DTColumnBuilder, DTInstances, dataservice, $translate) {
    ...
    vm.dtColumns.push(DTColumnBuilder.newColumn('DefaultPayment').withTitle('{{"FCC_LIST_COL_DEFAULT_PAYMENT" | translate}}').renderWith(function (data, type) {
        if (data == 'True') {
            return '<span class="text-success"> Mặc định </span>';
        }
        else {
