var ctxfolder = "/views/admin/task015";
...
app.factory('dataservice', function ($http) {
    ...
    return {
        insert: function (data, callback) {
            $http.post('/Admin/task015/insert', data).success(callback);
        },
        update: function (data, callback) {
            $http.post('/Admin/task015/update', data).success(callback);
        },
        //deleteItems: function (data, callback) {
        //    $http.post('/LgVendor/deleteItems', data).success(callback);
        //},
        delete: function (data, callback) {
            $http.post('/Admin/task015/delete/' + data).success(callback);
        },
        getItem: function (data, callback) {
            $http.get('/Admin/task015/getitem/' + data).success(callback);
        },
       
        gettreedataParent_id: function (callback) {
            $http.post('/Admin/task015/gettreedataParent_id/').success(callback);
        },
    }
});
...
app.controller('Ctrl_ESEIM', function ($scope, $rootScope, $compile, $uibModal, dataservice) {
    ...
    $rootScope.validationOptions = {
        ...
        messages: {
            code: {
                required: "Nhập mã",
                maxlength: "Mã không vượt quá 50 kí tự"
            },
            value: {
                required: "Nhập loại giá trị",
                maxlength: "Loại giá trị không vượt quá 50 kí tự"
            },
...
app.controller('index', function ($scope, $rootScope, $compile, $confirm, $uibModal, DTOptionsBuilder, DTColumnBuilder, DTInstances, dataservice, $filter) {
    ...
    vm.dtColumns.push(DTColumnBuilder.newColumn('value').withTitle('Quốc gia').renderWith(function (data, type) {
        return data;
    }));
    ...
    $scope.contextMenu = [
        [function ($itemScope) {
            return '<i class="fa fa-edit"></i>Cập nhật quốc gia ';
        ...
        [function ($itemScope) {
            return '<i class="fa fa-remove"></i> Xóa quốc gia';
        }, function ($itemScope, $event, model) {
