var ctxfolder = "/views/admin/task006";
...
app.factory('dataservice', function ($http) {
    ...
    return {
        insert: function (data, callback) {
            $http.post('/Admin/task006/insert', data).success(callback);
        },
        update: function (data, callback) {
            $http.post('/Admin/task006/update', data).success(callback);
        },
        //deleteItems: function (data, callback) {
        //    $http.post('/LgVendor/deleteItems', data).success(callback);
        //},
        delete: function (data, callback) {
            $http.post('/Admin/task006/delete/' + data).success(callback);
        },
        getItem: function (data, callback) {
            $http.get('/Admin/task006/getitem/' + data).success(callback);
        },
        gettreedataIdstudentb: function(callback) {
            $http.post('/Admin/task006/gettreedataIdstudentb/').success(callback);
        },

        gettreedataRelationship: function (data, callback) {
            $http.post('/Admin/task006/gettreedataRelationship?idstudentb=' + data).success(callback);
          
        },

        gettreedataLevel: function(callback) {
            $http.post('/Admin/task006/gettreedataLevel/').success(callback);
        },
    }
});
...
app.controller('index', function ($scope, $rootScope, $compile, $confirm, $uibModal, DTOptionsBuilder, DTColumnBuilder, DTInstances, dataservice, $filter) {
    ...
    $scope.initData();
    $scope.loadQuanHe = function () {
        dataservice.gettreedataRelationship($scope.model.idstudentb, function (result) {
            $scope.treedataRelationship = result.Object;
        });
    }
    ...
    vm.dtColumns.push(DTColumnBuilder.newColumn('firstname').withTitle('Họ tên').renderWith(function (data, type) {
        return data;
    }));
    vm.dtColumns.push(DTColumnBuilder.newColumn('idrelative').withTitle('Quan hệ').renderWith(function (data, type) {
        return data;
    }));
    vm.dtColumns.push(DTColumnBuilder.newColumn('idlevel').withTitle('Cấp học').renderWith(function (data, type) {
        return data;
    }));
    ...
    $scope.contextMenu = [
        [function ($itemScope) {
            ...
            return '<i class="fa fa-edit"></i>Cập nhật anh chị em ';
        }, function ($itemScope, $event, model) {
            debugger
            $scope.model.idstudentb = $itemScope.data.idstudentb;
            $scope.loadQuanHe();
        ...
        [function ($itemScope) {
            return '<i class="fa fa-remove"></i> Xóa anh chị em này';
        }, function ($itemScope, $event, model) {
        ...
        if ($scope.addform.validate()) {
            dataservice.insert($scope.model, function (rs) {
                ...
                if (rs.Error) {
                    ...
                    $scope.model.idstudentb = null;
                    $scope.model.idrelative = null;
