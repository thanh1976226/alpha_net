var ctxfolder = "/views/admin/proporCurrent";
...
app.factory('dataservice', function ($http) {
    ...
    return {
        getListArea: function (callback) {
            $http.post('/Admin/ProporCurrent/GetListArea/').success(callback);
        },
    }
});
...
app.controller('index', function ($scope, $rootScope, $confirm, $compile, $uibModal, DTOptionsBuilder, DTColumnBuilder, DTInstances, dataservice, $http, $filter) {
    ...
    vm.dtOptions = DTOptionsBuilder.newOptions()
        .withOption('ajax', {
            url: "/Admin/ProporCurrent/Jtable",
    ...
    //        return '';
    //}));
    vm.dtColumns.push(DTColumnBuilder.newColumn('Proportion').withTitle('Tỷ trọng').notSortable().withOption('sClass', 'tcenter').renderWith(function (data, type) {
        if (data == "0" || data == "") {
            return '<span class="text-danger bold"> 0%</span>';
        } else {
            if (data >= 50) {
                ...
                return '<span class="text-warning bold"> ' + data + '%</span>';
            }
        }
    }));
    vm.dtColumns.push(DTColumnBuilder.newColumn('TotalCanImp').withTitle('Trữ lượng trống (T)').notSortable().withOption('sClass', 'tcenter').renderWith(function (data, type) {
        if (data == "") {
            return '';
        } else {
    ...
    $scope.export = function () {
        ...
        location.href = "/Admin/ProporCurrent/ExportExcel?"
            + "page=" + page
            + "&row=" + length
